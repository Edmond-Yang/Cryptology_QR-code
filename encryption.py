...
def main():
	...
	if parties > 2:
		...
		questions = [
			inquirer.List('threshold',
				message='How many shares should be enough for decryption? (Most secure: ' + str(
				max_threshold) + ')',
			    choices=thresholds,
			),
		]
	...
	format = inquirer.prompt(questions, raise_keyboard_interrupt=True)['format']
	...
	for share in shares:  # Create png for each share
		...
		if format == 'png':
			...
			print(share[0] + '.png is Created.')
		elif format == 'svg':
			img.svg(share[0] + '.svg', scale=scale)
			print(share[0] + '.svg is Created.')
		elif format == 'terminal':
			print(img.terminal())
			print(share[0] + ' is Created.')
